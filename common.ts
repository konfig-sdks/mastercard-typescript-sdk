...
import { readableStreamToString, MastercardError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new MastercardError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new MastercardError(e, e.response?.data)
                }
                if (innerError instanceof MastercardError) {
                    // Got "MastercardError" from the above try block
                    throw innerError;
                }
